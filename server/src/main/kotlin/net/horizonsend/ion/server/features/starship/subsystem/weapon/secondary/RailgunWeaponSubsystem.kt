...
class RailgunWeaponSubsystem(
	...
	override fun fire(loc: Location, dir: Vector, shooter: Damager, target: Vector) {
		...
		Tasks.syncDelay((20.0 * balancing.warmupTime).toLong()) {
			...
			RailgunFireShockwaveAnimation().schedule()
	...
	fun turnBulbOn(bulbIndex: Int, location: Location){
		val newPos = pos.toLocation(location.world).add(face.direction.multiply((1+bulbIndex)))
	...
	fun turnAllBulbsOff(location: Location){
		for(i in 0..2){
			val newPos = pos.toLocation(location.world).add(face.direction.multiply((1+i)))
			val bulb = newPos.block.blockData as CopperBulb
			bulb.isLit = false
			newPos.block.blockData = bulb
		}
	...
	inner class RailgunFireShockwaveAnimation() : BukkitRunnable() {
