...
object MassShadows {
	...
	fun find(world: World, x: Double, z: Double): List<MassShadowInfo>? {
		...
		for (locusScheduler in AISpawners.getAllSpawners().mapNotNull { it.scheduler as? LocusScheduler }) {
			...
			allMassShadows.add(MassShadowInfo(text("AI locus"), center.blockX, center.blockZ, locusScheduler.radius.roundToInt(), dist.toInt(), 10000000))
		}

        return if (allMassShadows.isEmpty()) null
        else allMassShadows
	}
}
